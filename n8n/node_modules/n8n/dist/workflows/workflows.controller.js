...
let WorkflowsController = class WorkflowsController {
        ...
        const savedWorkflow = await Db.transaction(async (transactionManager) => {
            ...
            if (parentFolderId) {
                try {
                    ...
                    await transactionManager.update(workflow_entity_1.WorkflowEntity, { id: workflow.id }, { parentFolder });
                }
                catch { }
            }
            ...
            return await this.sharedWorkflowRepository.findWorkflowForUser(workflow.id, req.user, ['workflow:read'], { em: transactionManager, includeTags: true, includeParentFolder: true });
        });
        if (!savedWorkflow) {
    ...
    async getAll(req, res) {
        try {
            const { workflows: data, count } = await this.workflowService.getMany(req.user, req.listQueryOptions, !!req.query.includeScopes, !!req.query.includeFolders);
            res.json({ count, data });
        }
        catch (maybeError) {
    ...
    async getWorkflow(req) {
        ...
        if (this.license.isSharingEnabled()) {
            ...
            const workflow = await this.sharedWorkflowRepository.findWorkflowForUser(workflowId, req.user, ['workflow:read'], { includeTags: !this.globalConfig.tags.disabled });
            if (!workflow) {
    ...
    async transfer(req, _res, workflowId, body) {
        return await this.enterpriseWorkflowService.transferOne(req.user, workflowId, body.destinationProjectId, body.shareCredentials);
    }
};
