...
let ActiveWorkflowManager = class ActiveWorkflowManager {
    constructor(logger, errorReporter, activeWorkflows, activeExecutions, externalHooks, nodeTypes, webhookService, workflowRepository, orchestrationService, activationErrorsService, executionService, workflowStaticDataService, activeWorkflowsService, workflowExecutionService, instanceSettings, publisher, workflowsConfig) {
        this.logger = logger;
    ...
    async activateWorkflow(workflowId, activationMode) {
        ...
        try {
            ...
            if (wasActivated) {
                ...
                this.logger.debug(`Successfully started workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`, {
    ...
    async add(workflowId, activationMode, existingWorkflow, { shouldPublish } = { shouldPublish: true }) {
        ...
        try {
            ...
            if (['init', 'leadershipChange'].includes(activationMode) && !dbWorkflow.active) {
                this.logger.debug(`Skipping workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} as it is no longer active`, {
                    workflowId: dbWorkflow.id,
                });
                return false;
            }
            if (shouldDisplayActivationMessage) {
                this.logger.debug(`Initializing active workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} (startup)`, {
                    workflowName: dbWorkflow.name,
                    workflowId: dbWorkflow.id,
                });
            }
    ...
    async addTriggersAndPollers(dbWorkflow, workflow, { activationMode, executionMode, additionalData, }) {
        ...
        if (workflow.getTriggerNodes().length !== 0 || workflow.getPollNodes().length !== 0) {
            this.logger.debug(`Adding triggers and pollers for workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`);
            ...
            this.logger.debug(`Workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} activated`, {
                workflowId: dbWorkflow.id,
                workflowName: dbWorkflow.name,
            });
        }
